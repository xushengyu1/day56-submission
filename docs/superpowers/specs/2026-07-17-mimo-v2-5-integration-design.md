...
请求只发送已由失主确认的隐藏描述。模型返回 `questions`，每题必须包含：

- `question_text`
- `answer_key`

题目维度由服务端按顺序固定为 `question_1`、`question_2`、`question_3`，并统一标记为开放题，避免把模型生成的不稳定控制字段直接纳入领域数据。若问题文本泄露了参考答案，服务端将其替换为不含答案的通用提问；题目数量或必填文本不符合规则时整体失败，不落入半成品问题集。参考答案只保存在私有服务端数据中，不通过公开问题接口返回。
...
客户端设置固定超时和最多一次重试。三类调用使用 `max_completion_tokens=1024`，并通过 MiMo 的 OpenAI 兼容扩展关闭思考模式，避免推理内容占用完成令牌而截断 JSON；不增加其他动态参数。
