...
**文档版本：** V0.8（身份证件分支与多模态识别需求迭代）
**基线状态：** V0.8 已确认（2026-07-16），替代 V0.7
**项目周期：** 2 天
**项目人数：** 3 人
**产品形态：** Web 应用 Demo（React SPA 前端 + FastAPI 服务端）
**当前阶段：** PRD V0.8 与 Design V4 已确认，进入详细设计阶段
**核心目标：** 通过多模态信息提取和文本向量匹配提升发现效率，并针对身份证件类与其他物品采用不同的隐私保护和认领核验规则。
...
本产品是一套面向校园场景的 **AI 失物招领匹配与认领复核 Web 系统**。物品首先分为“身份证件类”和“其他物品”，系统通过多模态图片信息提取、拾得者人工确认、`PUBLIC + MATCH_ONLY` 文本向量化候选匹配、分类型认领核验、风险分流和统一管理员复核建立闭环：

> 注册登录 → 拾得者上传图片 → 多模态模型提取物品名称与特征 → 拾得者修改并确认 → 生成候选 → 身份证件按证件号码精确核验 / 其他物品按隐藏特征核验 → 通过后进入待交接 / 异常转管理员 → 拾得者确认交接 → 全流程留痕
...
| 对象 | 需要解决的问题 |
|---|---|
| 失主 | 如何用时间、地点、物品和描述找到真实候选；如何安全提交完整证件号码或回答隐藏问题；可选旧照片如何保存且不造成隐私泄露 |
...
flowchart TB
    ...
    subgraph Client["Web 前端（React 18 + Vite）"]
        UI1["注册登录与记录发布"]
        UI2["候选展示与隐藏核验"]
        UI3["异常复核与交接状态"]
    end
    ...
    subgraph Backend["Python + FastAPI 服务端"]
        ...
        AUDIT["操作日志与结果快照"]
        FILE["图片上传、脱敏副本与访问控制"]
        TYPE["物品类型分流<br/>身份证件 / 其他"]
    end

    subgraph AI["AI 能力（Embedding + 多模态大模型 + 文本核验 LLM）"]
        SEM["公开描述语义比较（text-embedding-v4）"]
        VISION["图片识别与结构化提取<br/>名称 / 公开特征 / 证件字段"]
        GUIDE["隐藏问题自动生成（mimoV2.5-pro）"]
        VERIFY["认领回答语义核验（mimoV2.5-pro）"]
    end

    subgraph Data["数据层"]
        DB[("PostgreSQL + pgvector<br/>四级结构化业务数据 + 文本向量<br/>候选、认领、审核、日志")]
        IMG[("图片文件存储<br/>PRIVATE 原图 + PUBLIC 脱敏副本")]
    end
...
- 拾得者上传的图片进入隔离的多模态提取流程；模型结果只是草稿，必须由拾得者修改或确认后才能发布。
- 候选匹配服务只读取拾得者确认后的 `PUBLIC + MATCH_ONLY` 文本，不直接读取原始图片、`VERIFICATION` 或完整证件号码。
- 回答核验 AI 只在隔离流程中比较拾得者标准答案与失主回答，普通用户接口不得返回标准答案。
- 身份证件原图和完整证件号码属于 `PRIVATE`；候选页只能展示掩码号码和脱敏图片副本。完整号码仅用于认领时的服务端精确比对。
- PostgreSQL 保存四级结构化业务数据、证件号码比对值和审计数据；pgvector 保存拾得者确认后的公开文本向量。隐藏特征和证件号码必须通过服务端权限控制与公开响应模型隔离。
...
> 作为管理员，我希望系统先自动分流高可信申请，只把冲突和异常申请送入队列；审核时能够同时看到原始输入、AI 分析与隐藏特征核验证据，并让每次决定都有理由和时间记录。
...
7. 其他物品要求拾得者填写隐藏特征；失主回答 LLM 根据隐藏特征生成的开放式问题，达到安全门槛后进入“待交接”，否则进入管理员复核。
...
11. 多模态提取超时、非法结构、低置信度或图片不可读时，系统保留原图与草稿，要求拾得者手工补全；核验 AI 异常时进入管理员复核，不伪造结果。
12. MVP 至少演示身份证件号码匹配主路径、其他物品隐藏核验主路径、号码错误/AI 识别错误等失败场景，以及完整证据链。
...
系统保留该候选。失主准确回答 LLM 根据隐藏特征生成的验证问题，且候选分、隐藏核验分和综合认领可信度均达到高可信门槛后，系统自动进入”待交接”。
...
3. 物品类型必须一致：身份证件类只与身份证件类匹配，其他物品不进入证件号码核验分支。
4. 已认领、已撤销或已关闭记录不参与匹配。
...
隐藏核验分由 LLM 生成的每个问题的 `匹配 / 部分匹配 / 冲突 / 无法判断` 结果汇总。问题等权。
...
多模态提取结果只有经拾得者确认后才能作为公开文本参与候选分；图片本身不参与认领可信度。该权重和阈值仅用于 `OTHER` Demo，必须通过测试样例验证。
...
4. 未超过尝试次数，不存在多人同时认领、账号风险、号码解析异常或比对服务异常。

身份证件分支不调用隐藏问题 LLM、不计算隐藏核验分，也不把完整号码交给模型。条件满足后进入待交接；失败不直接透露差异，异常进入管理员复核。
...
身份证件分支将“AI核验中”替换为“号码核验中”；精确匹配成功直接进入待交接，失败/超限/异常进入待管理员复核或终止本次申请，不返回号码差异。
...
PRD V0.6 曾由用户确认，V0.7 同步了上一轮方案。2026-07-16 用户新增身份证件/其他双类型、多模态图片识别和证件号码认领要求，形成 V0.8。用户随后确认 Q1～Q15 与方案 A，V0.8 正式替代 V0.7，进入详细设计阶段。

### 15.2 已确认设计约束与待验证效果

下列范围和规则已经确认，不再作为 PRD 待确认项：

1. OTHER 自由文本核验采用“确定性输入检查 + AI 语义比对 + 安全门槛”，任何冲突、无法判断或 AI 异常都转管理员。
2. Demo 初始阈值保留为候选分 `80`、隐藏核验分 `85`、综合认领可信度 `85`；阈值效果仍需用测试样例校准。
...
> **身份证件类以完整号码精确匹配作为认领核验，其他物品以隐藏特征核验作为认领核验；两条路径通过后都只进入“待交接”。最终“已认领”仍只能由拾得者在线下交接后确认。**
